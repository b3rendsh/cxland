...
;
; MSX_NOTE: PPI IDE INTERFACE IS CALLED THE BEER IDE OR BEER 202 CART
; THE MSX BEER CART ONLY SUPPORTS A MASTER UNIT, NO SLAVE UNIT
...
PPIDE_DEV0S:	; DEVICE 0, SLAVE
	.DB	$FE			; DRIVER DEVICE NUMBER (FILLED DYNAMICALLY)
#IFDEF MSX_MOD
	.DB	PPIDE_STNOMEDIA		; THE MSX BEER CART ONLY SUPPORTS A MASTER UNIT, NO SLAVE UNIT
#ELSE
	.DB	0			; DEVICE STATUS
#ENDIF
	.DB	0			; DEVICE TYPE
...
; MSX_NOTE: THE MSX BEER CART ONLY SUPPORTS A MASTER UNIT, NO SLAVE UNIT
#IFNDEF MSX_MOD	
...
;
; MSX_NOTE: SETTING THE DIRECTION ON A BEER CART WILL RESET THE CONTROL SIGNALS.
#IFDEF MSX_MOD
	LD	A,$A5
	CALL	PPIDE_OUT		; THIS ROUTINE WILL USE THE CORRECT SIGNAL CONTROL SEQUENCE
	.DB	PPIDE_REG_DATA
...
#ENDIF
;
; MSX_NOTE: IT IS REQUIRED TO FIRST SETUP THE PPI TO READ THEN WAIT FOR BUFFER
#IFDEF MSX_MOD
	; SETUP PPI TO READ
...
PPIDE_GET8:	; 8 BIT WIDE READ LOOP
	...
	RET
;
; MSX_NOTE: TODO, THIS LOOP CAN BE OPTIMIZED FOR FASTER THROUGHPUT.
...
#IFDEF MSX_MOD
; MSX_NOTE: SETTING THE DIRECTION ON A BEER CART WILL RESET THE CONTROL SIGNALS.
; THE WORKAROUND FOR WRITE OPERATIONS IS TO USE A SMALL DELAY INSTEAD OF WAITING FOR BUFFER READY.
;
	; FIRST SETUP PPI TO WRITE
	LD	A,PPIDE_DIR_WRITE	; SET DATA BUS DIRECTION TO WRITE
	;OUT	(PPIDE_REG_PPI),A	; DO IT
	LD	C,(IY+PPIDE_PPI)	; PPI CONTROL WORD
	EZ80_IO
	OUT	(C),A			; WRITE IT
;
	; THEN SMALL DELAY FOR BUFFER READY
	PUSH	BC
	LD	B,$30
WR_WAIT:
	EX	(SP),HL
...
;
	...
	XOR	PPIDE_CTL_DIOW		; SWAP THE WRITE LINE BIT (MSX_NOTE: COMMENT TYPO, WRITE NOT READ)
...
;
; MSX_NOTE: FIRST SEND DATA VALUE THEN ASSERT WRITE LINE
#IFDEF MSX_MOD
	DEC	C
	DEC	C
	EZ80_IO
